...
func (d *Detector) DetectCommandOutput(responseContent string, baselineContent string) (bool, string) {
	...
	highConfidencePatterns := []struct {
		...
		{`nobody:[^:]*:\d+:\d+:`, "/etc/passwd内容", 0.95},
		{`www-data:[^:]*:\d+:\d+:`, "/etc/passwd内容", 0.95},
		{`Directory of\s+[A-Z]:\\[^\n]+\n\n`, "Windows dir输出", 0.90},
		{`Volume Serial Number is [A-Z0-9-]+`, "Windows vol输出", 0.95},
		{`Linux\s+\S+\s+\d+\.\d+\.\d+`, "uname -a输出", 0.85},
		{`Darwin\s+\S+\s+\d+\.\d+\.\d+`, "macOS uname输出", 0.85},
		{`Windows\s+\S+\s+\[Version`, "Windows版本信息", 0.95},
		{`Microsoft\s+Windows\s+\[Version`, "Windows版本", 0.95},
		{`flag\{[^}]+\}`, "CTF flag格式", 0.99},
		{`FLAG\{[^}]+\}`, "CTF flag格式", 0.99},
		{`ctf\{[^}]+\}`, "CTF flag格式", 0.99},
		{`CTF\{[^}]+\}`, "CTF flag格式", 0.99},
		{`hctf\{[^}]+\}`, "HCTF flag", 0.99},
		...
		{`qwb\{[^}]+\}`, "强网杯 flag", 0.99},
		{`key\{[^}]+\}`, "key格式", 0.90},
		{`secret\{[^}]+\}`, "secret格式", 0.90},
		{`BEGIN\s+RSA\s+PRIVATE\s+KEY`, "RSA私钥泄露", 0.99},
		{`BEGIN\s+PRIVATE\s+KEY`, "私钥泄露", 0.99},
		{`BEGIN\s+OPENSSH\s+PRIVATE\s+KEY`, "SSH私钥泄露", 0.99},
		{`ssh-rsa\s+AAAA`, "SSH公钥", 0.90},
		{`ping\s+statistics`, "ping命令输出", 0.85},
		{`bytes from\s+\S+:`, "ping响应", 0.80},
		{`ttl=\d+`, "ping TTL", 0.75},
		{`HAWKEYE_TEST_\d+`, "HawkEye测试标记", 0.99},
		{`RCE_TEST_MARKER_\d+`, "RCE测试标记", 0.99},
		{`PHP\s+Version`, "phpinfo输出", 0.95},
		{`PHP Version`, "phpinfo输出", 0.95},
		{`phpinfo\(\)`, "phpinfo函数调用", 0.90},
		{`Configuration\s+Command`, "PHP配置信息", 0.85},
		{`Server\s+API`, "PHP Server API", 0.85},
		{`Loaded\s+Configuration\s+File`, "PHP配置文件路径", 0.85},
		{`array\(\d+\)\s+\{`, "PHP var_dump输出", 0.80},
		{`string\(\d+\)\s+"`, "PHP var_dump字符串", 0.80},
		{`int\(\d+\)`, "PHP var_dump整数", 0.75},
		{`bool\(true\)`, "PHP var_dump布尔值", 0.75},
		{`\/\S+\.php`, "PHP文件路径", 0.70},
		{`\/var\/www\/html`, "Web目录路径", 0.80},
		{`\/home\/\w+`, "用户目录路径", 0.70},
		{`Array\s*\(\s*\[`, "PHP数组输出", 0.75},
		{`stdClass\s+Object`, "PHP对象输出", 0.80},
		{`Proto\s+Local\s+Address`, "netstat表头", 0.85},
		{`Active\s+Internet\s+connections`, "netstat输出", 0.85},
		{`USER\s+PID\s+%CPU`, "ps表头", 0.85},
		{`PID\s+TTY\s+TIME`, "ps输出", 0.85},
		{`Tasks:\s+\d+\s+total`, "top任务统计", 0.85},
		{`%Cpu\(s\):`, "CPU使用率", 0.85},
		{`MiB\s+Mem\s+:`, "内存信息", 0.85},
		{`eth\d+:\s+flags`, "网卡信息", 0.80},
		{`inet\s+\d+\.\d+\.\d+\.\d+`, "IP地址", 0.75},
		{`ether\s+[0-9a-f:]+`, "MAC地址", 0.80},
		{`PATH=`, "环境变量PATH", 0.70},
		{`HOME=`, "环境变量HOME", 0.70},
		{`USER=`, "环境变量USER", 0.70},
		{`SHELL=`, "环境变量SHELL", 0.70},
		{`Apache\s+\d+\.\d+`, "Apache版本", 0.85},
		{`nginx/\d+\.\d+`, "Nginx版本", 0.85},
		{`PHP\s+\d+\.\d+`, "PHP版本", 0.85},
		{`mysql\s+Ver`, "MySQL版本", 0.85},
		{`Python\s+\d+\.\d+`, "Python版本", 0.85},
		{`Ruby\s+\d+\.\d+`, "Ruby版本", 0.85},
		{`Java\s+\d+`, "Java版本", 0.85},
		{`Node\.js\s+v?\d+`, "Node.js版本", 0.85},
		{`Docker\s+version`, "Docker版本", 0.85},
		{`git\s+version`, "Git版本", 0.85},
		{`openssl\s+\d+\.\d+`, "OpenSSL版本", 0.85},
	}
...
func (d *Detector) AnalyzeResponse(response map[string]interface{}, p types.Payload, baselineResponse map[string]interface{}) *types.Vulnerability {
	...
	if p.PayloadType == types.PayloadTypeTimeBased {
		...
		if baselineResponse != nil {
			if elapsed, ok := baselineResponse["elapsed"].(float64); ok {
	...
	if p.PayloadType == types.PayloadTypeEchoBased || p.PayloadType == types.PayloadTypeCodeExec {
		...
		if p.ExpectedOutput != "" && strings.Contains(content, p.ExpectedOutput) {
			baselineHasExpected := baselineContent != "" && strings.Contains(baselineContent, p.ExpectedOutput)
			fmt.Printf("[DEBUG] AnalyzeResponse: ExpectedOutput=%s, ContentLen=%d, BaselineLen=%d, BaselineHas=%v\n", 
				p.ExpectedOutput, len(content), len(baselineContent), baselineHasExpected)
			if !baselineHasExpected {
				return d.createVulnerability(
					targetURL,
					"",
					p,
		...
		if p.PayloadType == types.PayloadTypeCodeExec {
			if len(content) > 0 && len(baselineContent) > 0 {
				lenDiff := len(content) - len(baselineContent)
				if lenDiff > 10 || lenDiff < -10 {
					return d.createVulnerability(
						targetURL,
						"",
						p,
						types.SeverityMedium,
						fmt.Sprintf("响应长度差异: %d -> %d (差异: %d)", len(baselineContent), len(content), lenDiff),
						"响应内容变化可能表明代码被执行",
					)
				}
			}
