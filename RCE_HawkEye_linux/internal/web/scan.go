...
func (s *Server) runScan(ctx context.Context, scanID string, req ScanRequest) {
	...
	initialHistory := types.ScanHistory{
		ID:        scanID,
		Target:    targetURL,
		Domain:    domain,
		Status:    "running",
	...
	if err := historyMgr.Add(initialHistory); err != nil {
		fmt.Printf("[!] Failed to create initial history: %v\n", err)
	}
	...
	progressCallback := func(current, total int, targetURL string, extra map[string]interface{}) {
		...
		if extra != nil {
			if phase, ok := extra["phase"].(string); ok {
				switch phase {
				case "start":
					params, _ := extra["params"].(int)
					payloads, _ := extra["payloads"].(int)
					taskDetail = fmt.Sprintf("Starting: %d params, %d payloads", params, payloads)
				case "response":
					param, _ := extra["param"].(string)
					method, _ := extra["method"].(string)
					payloadPreview := ""
					if p, ok := extra["payload"].(string); ok {
						if len(p) > 30 {
							payloadPreview = p[:30] + "..."
						} else {
							payloadPreview = p
					...
					taskDetail = fmt.Sprintf("[%s] %s=%s", method, param, payloadPreview)
				case "tasks_created":
					taskCount, _ := extra["task_count"].(int)
					taskDetail = fmt.Sprintf("Created %d scan tasks", taskCount)
				case "payloads_final":
					count, _ := extra["count"].(int)
					taskDetail = fmt.Sprintf("Loaded %d payloads", count)
				default:
					taskDetail = fmt.Sprintf("Phase: %s", phase)
				}
			}
		}
		
		if taskDetail == "" {
			taskDetail = fmt.Sprintf("Scanning %s", truncateURL(targetURL, 50))
		}
	...
	if err != nil {
		...
		fmt.Printf("[DEBUG] Report saved: VulnCount=%d, Format=%s\n", len(allVulns), outputFormat)
	}
	...
	fmt.Printf("[DEBUG] Scan completed. Total vulnerabilities: %d\n", len(allVulns))
	for i, v := range allVulns {
		fmt.Printf("[DEBUG] Vuln %d: Target=%s, Param=%s, Payload=%s, Severity=%s\n", i, v.Target, v.Parameter, v.Payload, v.Severity)
	}
	...
	finalDomain := domain
	if len(allTargets) > 0 && finalTargetURL == "" {
		finalTargetURL = allTargets[0].URL
		if parsedURL, err := url.Parse(finalTargetURL); err == nil {
			finalDomain = parsedURL.Host
		}
	}
	...
	scanHistory := types.ScanHistory{
		...
		EndTime:       endTime,
		Duration:      duration,
		VulnCount:     len(allVulns),
		CriticalVulns: criticalVulns,
		HighVulns:     highVulns,
		MediumVulns:   mediumVulns,
		LowVulns:      lowVulns,
		Vulnerabilities: allVulns,
		ScanLevel:     scanLevelInt,
		ScanMode:      string(scanMode),
	}
	
	if err := history.GetManager().Update(scanHistory); err != nil {
		fmt.Printf("[!] Failed to update history: %v\n", err)
	} else {
		fmt.Printf("[DEBUG] History updated: ID=%s, VulnCount=%d\n", scanID, len(allVulns))
	}
}
